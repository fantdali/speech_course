...
class FastPitch(nn.Module, DeviceGetterMixin):
    def __init__(self, hparams: HParamsFastpitch):
        ...
        self.pad_token_id = SymbolsSet().symbols_to_id['AA0']
    ...
    def get_encoder_out(self, batch: FastPitchBatch):
        ...
        '''
        # <YOUR CODE HERE>
        text_emb = self.symbol_emb(batch.texts)  # (batch, len(text), symbols_embedding_dim) 
        enc_out, enc_mask = self.encoder(text_emb, batch.text_lengths)  # (batch, len(text), symbols_embedding_dim)
    ...
    def forward(self, batch: FastPitchBatch, use_gt_durations=True, use_gt_pitch=True, max_duration=75):
        ...
        pitch_pred = self.pitch_predictor(enc_out, enc_mask)  # (batch, len(text))
        log_dur_pred = self.duration_predictor(enc_out, enc_mask)  # (batch, len(text))
        dur_pred = torch.clamp(torch.exp(log_dur_pred), 0, max_duration)  # (batch, len(text))
        ...
        enc_rep, mel_lens, _ = regulate_len(dur_pred, enc_out)

        mel_out, _ = self.decoder(enc_rep, mel_lens) 
        mel_out = self.proj(mel_out)  # (batch, time, mel_bins)
    ...
    def infer_encoder(self, batch: FastPitchBatch, max_duration=75):
        ...
        pitch_pred = self.pitch_predictor(enc_out, enc_mask).unsqueeze(1)  # (batch, 1, len(text))
        log_dur_pred = self.duration_predictor(enc_out, enc_mask)
        dur_pred = torch.clamp(torch.exp(log_dur_pred), 0, max_duration)  # (batch, len(text), 1)
