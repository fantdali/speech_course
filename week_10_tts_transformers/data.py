...
class BioembModel:
    ...
    def __init__(self, device, token=None):
        self.device = device
        self.speaker_embedder = Model.from_pretrained(
            checkpoint="pyannote/embedding",
            token=token,
        ).to(self.device)
        self.speaker_embedder = Inference(
            self.speaker_embedder, window="whole", device=device
        )
        if isinstance(self.speaker_embedder.model.specifications, torch.nn.Identity):
            self.speaker_embedder.model.specifications = None
    ...
    def __call__(self, waveform, sr):
        ...
        while waveform_for_loud.shape[0] <= self.SR:
            waveform_for_loud = np.concatenate(
                (waveform_for_loud, waveform_for_loud), axis=0
            )
        ...
        embedding = self.speaker_embedder(
            {
                "waveform": torch.tensor(
                    waveform, device=self.device, dtype=torch.float
                ).unsqueeze(dim=0),
                "sample_rate": sr,
            }
        )
...
class CodecApplier:
    def __init__(
        self, config_path, ckpt_path, sample_rate=16000, device=torch.device("cpu")
    ):
        self.device = device
    ...
    def encode(self, waveform, sample_rate):
        ...
        waveform = librosa.resample(
            waveform, orig_sr=sample_rate, target_sr=self.sample_rate
        )
